...
class Program(object):
    '''A program.'''
    def __init__(self, name, weight, children, tower_weight):
        self.name = name
        self.weight = weight
    ...
    def __repr__(self):
        ...
        return f'Program({self.name}, {self.weight}, {self.tower_weight}, [{children}])'
...
def parse_input(input_data):
    '''Turn input data string into data format.'''
    items = (x.strip() for x in input_data.strip().split('\n'))
    return list(map(create_program, items))


def create_program(input_data):
    '''Create a Program from an input string.'''
    if '->' in input_data:
        program, children = input_data.split('->')
        children = [x.strip() for x in children.split(',')]
    else:
        program, children = input_data.strip(), ''
        children = []
    name, weight = program.strip().split(' ')
    weight = int(weight.strip('()'))

    return Program(name, weight, children, 0)
